...
public static void captureScreenshot(int TCID) throws IOException {
	File destination=((TakesScreenshot)driver).getScreenshotAs(OutputType.FILE);
	File src=new File(System.getProperty("user.dir")+"//FailedTestcaseScreenshot//FailedTestCase_"+TCID+".jpg");
...
public static Object[][] getData(Method m) throws IOException
{
	...
	if(m.getName().equals("verifyLoginFunctionality"))
	{
	...
	else if(m.getName().equals("verifyItemAdded"))
	{
		ExcelUtility ex=new ExcelUtility(System.getProperty("user.dir")+"//TestData//Testdata.xlsx");
